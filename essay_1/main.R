...
# Creating the model
model <- lm(SAT ~ GPA, data = training_set)
...
cor(training_set$SAT, training_set$GPA) # returns correlation coefficient
...
# Visualization
scatter_plot <- ggplot(test_set, aes(x = SAT, y = GPA)) +
  geom_point() +
  geom_smooth(method = "lm", col = "turquoise") +
  ggtitle("SAT and GPA Scores with Regression Line")
